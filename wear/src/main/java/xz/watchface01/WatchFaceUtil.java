/* Utility functions to transfer data between handheld and wear app.
 * Modified on the demo from Google android studio tutorial.
 */
/*
...
public final class WatchFaceUtil {
    ...
    public static final String PATH_SUGGESTION_DATA = "/wear_data";
    public static final String KEY_SUGGESTION_DATA = "suggestion_data";
        ...
        Wearable.NodeApi.getLocalNode(client).setResultCallback(
                new ResultCallback<NodeApi.GetLocalNodeResult>() {
                    ...
                    public void onResult(NodeApi.GetLocalNodeResult getLocalNodeResult) {
                        ...
                        Uri uri = new Uri.Builder()
                                .scheme("wear")
                                .path(WatchFaceUtil.PATH_SUGGESTION_DATA)
                                .authority(localNode)
                                .build();
                        Wearable.DataApi.getDataItem(client,uri)
    ...
    private static class DataItemResultCallback implements ResultCallback<DataApi.DataItemResult> {
        ...
        public void onResult(DataApi.DataItemResult dataItemResult) {
            DataMap dataMap;
            /*if (!dataItemResult.getStatus().isSuccess()) return;
    ...
    // handling suggestion data

    public interface FetchSuggestionDataMapCallback{
        void onSuggestionDataMapFetched(DataMap suggestionDataMap);
    }

    public static void fetchSuggestionData(final GoogleApiClient client, final FetchSuggestionDataMapCallback callback){
        Wearable.NodeApi.getLocalNode(client).setResultCallback(
                new ResultCallback<NodeApi.GetLocalNodeResult>() {
                    @Override
                    public void onResult(NodeApi.GetLocalNodeResult getLocalNodeResult) {
    ...
    public static void overwriteSuggestionData(final GoogleApiClient client, final DataMap suggestionDataToOverwrite){
        WatchFaceUtil.fetchSuggestionData(client,
                new FetchSuggestionDataMapCallback() {
                    @Override
                    public void onSuggestionDataMapFetched(DataMap currentSuggestionData) {
                        DataMap overwrittenSuggestionData = new DataMap();
                        overwrittenSuggestionData.putAll(currentSuggestionData);
                        overwrittenSuggestionData.putAll(suggestionDataToOverwrite);
                        WatchFaceUtil.putSuggestionDataItem(client,suggestionDataToOverwrite);
                    }
                });
    }

    public static void putSuggestionDataItem(GoogleApiClient client, DataMap newSuggestionData){
        PutDataMapRequest putDataMapRequest = PutDataMapRequest.create(PATH_SUGGESTION_DATA);
        DataMap suggestionDataToPut = putDataMapRequest.getDataMap();
        suggestionDataToPut.putAll(newSuggestionData);
